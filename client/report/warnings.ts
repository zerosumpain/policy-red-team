...
const figure = (text: string) => Number(text.replace(/,/g, ''));
...
export function truncations(stages: { name: string; ordinal: number; warnings: string[] }[]): Truncation[] {
  ...
  for (const stage of stages) {
    ...
    for (const text of hits) {
      const here = [...text.matchAll(WITHHELD)].reduce((n, m) => Math.max(n, figure(m[1])), 0);
      ...
      clipped = Math.max(clipped, [...text.matchAll(CLIPPED)].reduce((n, m) => n + figure(m[1]), 0));
    }
    rows.push({
      name: stage.name,
