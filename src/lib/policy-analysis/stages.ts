...
export const STAGE_NOTES: StageNote[] = [
  ...
  { phase: 'write', name: 'Actor persona library', what: 'Matches the bodies in this paper to ones you have assessed before.',
    ...
    lands: 'Met before' },
  { phase: 'write', name: 'Theory of change', what: 'One logic model for the whole programme, then a close reading of the eight mechanisms the red team attacks most.',
    detail: 'The logic model runs from inputs through activities, outputs and outcomes to impacts for the policy as a whole. Then the mechanisms the most exploitation plays are aimed at each get a deep chain: the causal step at every jump, alternative explanations for the same outcome, pathways where the intervention makes things worse, and whether one of the plays breaks a step. Every chain names its weakest link.',
    given: 'Mechanisms, evidence and assumptions, and the plays aimed at each deep-chain mechanism', emits: 'A programme logic model · causal chains',
    limit: 'Only the most attacked mechanisms get a deep chain; the rest are covered by the logic model. Each states its own judgement — well supported, supported with limits, contested or provisional — by what the evidence shows.',
  ...
  { phase: 'write', name: 'Independent challenge', what: 'A second analytical pass whose only job is to attack the report it was handed.',
    detail: 'Eleven categories, with equal weight. Seven ask whether the report is wrong: omission, citation, causality, counter-evidence, confidence, recommendation, completeness. Four ask whether it is useful: is it generic, does it say who should do what, did it miss the sharpest play, and is a severe play left with no recommendation answering it. Each challenge records what was tested, the evidence for the objection, its materiality, and what resolution it would need — and where the problem is vagueness, the resolution asks for specificity, not caution.',
