import { ASSURANCE_CATEGORIES, JUDGEMENTS, REPORT_SECTIONS, type Artefact } from './contracts';
...
export function benchmarkRobustness(artefacts: Artefact[]): RobustnessBenchmark {
  ...
  const judged = [...chains, ...options, ...plans, ...assured].filter((a) => (JUDGEMENTS as readonly string[]).includes(String(a.data.judgement)));
  const analytical = [...chains, ...options, ...plans, ...challenges, ...responses, ...summaries, ...assured];
  const traced = analytical.filter((a) => reachesSource(a.id, map));

  const metrics: RobustnessMetric[] = [
    { key: 'causal_coverage', score: ratio(new Set(chains.map((a) => String(a.data.mechanismId))).size, mechanisms.length), detail: `${chains.length} chains for ${mechanisms.length} mechanisms` },
    ...
    { key: 'qualitative_judgement', score: ratio(judged.length, [...chains, ...options, ...plans, ...assured].length), detail: `${judged.length} of ${[...chains, ...options, ...plans, ...assured].length} analytical conclusions use the qualitative scale` },
