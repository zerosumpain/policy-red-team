...
export type ChangeStrip = {
  ...
  plays: number;
  steps: StripStep[];
  /** Index into `steps`, or null where no step is weak. */
  weakest: number | null;
...
function stepOf(key: StepKey, label: string, value: unknown): StripStep {
  const items = Array.isArray(value) ? value.map((v) => String(v).trim()).filter(Boolean) : [];
  const unspecified = items.filter(isUnspecified).length;
  const hedged = items.filter((item) => HEDGED.test(item)).length;
  const weak = items.filter((item) => isUnspecified(item) || HEDGED.test(item)).length;
  const state: StepState = !items.length || items.every(isBlank)
    ? 'missing'
...
export function stripOf(chain: Artefact): { steps: StripStep[]; weakest: number | null } {
  const steps = STEPS.map((step) => stepOf(step.key, step.label, chain.data?.[step.key]));
